...
const getWeatherData = async () =>{
    try {
        const weatherData = await axios.get(`${BASE_URL_ONECALL}?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=${API_KEY}&units=metric&lang=es`);
...
const removeCity = ()=>{
    const cities = JSON.parse(localStorage.getItem(LOCAL_SAVED_CITIES));
    const updateCities = cities.filter((city)=> city.id !== route.query.id);
    localStorage.setItem(LOCAL_SAVED_CITIES,JSON.stringify(updateCities));
    router.push({
        name: 'home',
    })
}
